...
def checkout_cart_safely(user):
    ...
    if not lock.acquire(blocking=False):
        print(f"[{server}] REDIS LOCK REJECTED checkout:{user.id}")
        return None, "Checkout already in progress", None

    try:
        print(f"[{server}] REDIS LOCK ACQUIRED checkout:{user.id}")
        ...
        if lock.owned():
            print(f"[{server}] REDIS LOCK RELEASED checkout:{user.id}")
            lock.release()
