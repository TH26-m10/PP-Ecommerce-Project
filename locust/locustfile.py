...
class AuthenticatedUser(HttpUser):
    ...
    def on_start(self):
        ...
        if token:
            ...
            print(f"Failed login for user {self.user_id}")


class StressCheckoutUser(AuthenticatedUser):
...
class StressBrowseUser(AuthenticatedUser):
    ...
    def best_sellers(self):
        self.client.get(
            ...
            name="/api/product/best-sellers"
        )

    @task(1)
    def lrt_status(self):
        self.client.get(
            "/api/load-balance/lrt-status",
            name="/api/load-balance/lrt-status"
        )
...
class BestSellerUser(AuthenticatedUser):
